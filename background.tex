...
However, Katt et al. \cite{katt2008general} argued that obligations can also express actions to be taken by the system itself rather than the subject.
...
The authors assume that the \(pre\) and \(post\) evaluations are duration-less, which means that \(pre\) and \(post\) rules are evaluated only once in a usage session.
...
In \cite{ucs2019ifttt}, La Marra et al. used \gls{ifttt} to express \gls{ucon} obligations, then they leveraged \gls{ucon} obligations to trigger \gls{ifttt} applets based on policy-evaluation, which demonstrates that \gls{ucon} can be used for use-cases that go beyond access control.
One example of such use-cases is a smart farm where the humidity and temperature are collected from sensors and used as attributes in \gls{ucon} to decides when to start watering or heating the plants.
...
It is much less verbose than \gls{xacml} and consequently more compact and shorter in size.
As such, \gls{alfa} enables a considerable improvement in performance and efficiency of policy parsing and evaluation.
...
The attribute is called \texttt{step} and takes one of the following three string values: ``\texttt{pre}'', ``\texttt{ongoing}'' or ``\texttt{post}''.
It is updated by the system as the usage session moves among the three phases.
