...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=\linewidth]{figures/uconabc_state_model.png}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=\linewidth]{figures/uconabc_ext_state_model.png}
...
. To realise the model, Colombo et al. [5] proposed a new policy language, U-XAMCL, that extends XACML semantics with UCON novelties and defines an architecture to enforce UCON policies.    This architecture has become the basis of all UCON implementations and applications (albeit with minor modifications), and it is sometimes designated as the Usage Control System (UCS). For instance, Lazouski [6] et al. developed a web-based UCS prototype that evaluates U-XACML policies and realises the UCS architecture. Another example is the usage control framework for cloud systems that was introduced by Carniani et al. [7]  to continuously regulate access to cloud resources. Likewise, a fault-tolerant decentralised peer-to-peer UCS framework for the Internet of Things (IoT) is presented in [8]. More importantly, a hierarchical architecture that improves the efficiency and performance of UCS is defined in [9]. Similarly, Martinelli et al. [10] extended UCS architecture to formalise the management of UCS obligations and enforce them. 
