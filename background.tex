...
Instead, we view them as atomic actions that trigger transitions among the \(pre\), \(ongoing\) and \(post\) phases, because transitions occur due to the decisions that are determined by policy evaluations. 
...
The resulting state model is an extension to the one described in Figure \ref{fig:ucon_cnr_state_model}, where local state transitions are added to the \(pre\), \(ongoing\) and \(post\) phases.
