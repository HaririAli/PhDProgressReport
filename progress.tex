...
\subsection{State and Session Management in Distributed \acrshort{ucsp}}\label{sec:disrtibution}
...
The objective is to allow a cluster of \acrshort{ucsp} nodes to work together and cooperate for handling in large number of usage sessions in cloud or enterprise environments.
...
Specifically, when a usage request is created or when an attribute is updated, a \gls{pmp} queues and evaluation job, and one \acrshort{ucsp} node picks the job, loads session information from the \gls{srp} then evaluates policies and updates the session information.
...
In addition, resource utilisation in this architecture is high, an load can be properly distributed and balanced, because \acrshort{ucsp} nodes are invoked when needed.
