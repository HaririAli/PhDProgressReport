...
(i.e., \(preA\), \(ongoingA\), \(preB\), \(ongoingB\), \(preC\) and \(ongoingC\)), where \(pre\) predicates are evaluated before usage starts while \(ongoing\) predicates are evaluated while usage is in progress.
...
Moreover, Katt et al. consider attributes as a special type of obligations and they argue that attribute updates can be expressed as obligations that must be performed by the system.
...
The extended policy language was designated as U-XACML and it introduces an implicit temporal state by classifying policy rule as \(pre\), \(ongoing\) or \(post\) rules such that \(pre\) rules are applicable before granting access, \(ongoing\) rules are applicable while access is in progress, and \(post\) rules are applicable after the end of access.
...
It also introduces internal attributes that are specific to a single usge session and managed internally as part of the session.
...
\gls{alfa} maps directly to \gls{xacml} and has the same semantics.
It conforms to the same hierarchical structure of \gls{xacml} which incorporates policy sets, policies and rules.
...
The attribute is called \texttt{step} and it takes one of the following three string values: \texttt{``pre''}, \texttt{``ongoing''} or \texttt{``post''}, and it is updated by the system as the usage session moves among the three states.
Therefore, rules can be classified by including a check on the value of the \texttt{step} attribute in each rule.
...
This maximises performance and minimizes the need for high network speeds or computational resources.
...
They also defined the state model of \gls{ucon} such that the state of a single usage process transitions among the following six states: \(initial\), \(requesting\), \(denied\), \(accessing\), \(revoked\), and \(end\), as shown in Figure \ref{fig:ucon_state_model}.
...
For this reason, they extended the state model by introducing the \(ongoing-check\) such that the system moves to this state when a change occurs while usage is in progress.
...
If a re-evaluation results into a \(deny\) decision or if the subject ends their usage, then the session moves to the exit state, or to the \(post\) state where \(post\) rules are evaluated and the corresponding obligations are enforced.
...
In \cite{ucs2020plus}, our team argues that denying access does not necessarily require terminating the authorisation process, but rather keep monitoring the state and re-evaluating when a change occurs.
...
The counter is also used in the default deny rule such that the default rule is not applicable as long as the counter is below a specific threshold.
Therefore, if access cannot be granted, \acrshort{ucsp} keeps monitoring the session and re-evaluating policies until the counter expires. 
The resulting state model is an extension to the one described in Figure \ref{fig:ucon_cnr_state_model}, where local state transitions are added to the \(pre\), \(ongoing\) and \(post\) states.
