...
\subsection{Foundational Objectives}\label{sec:foundational}
...
For this reason, continuity of these phases must be enabled even when access is \textit{denied}.
Secondly, the counter was added to allow monitoring specific attributes or specific changes such as trust level or user behaviour.
However, the counter may expire before capturing the changes of interest due to other changes or updates.
...
Accordingly, we intend to define multiple state models that fit different use-cases, and to allow \acrshort{ucsp} to dynamically select the suitable state model based on the configuration or on the application.


\subsubsection{Additional Classifications}\label{sec:phases}
Our team's extension of \acrshort{ucsp} state model in \cite{ucs2020plus} allows the session to stay in the \(ongoing\) phase when the decision is \(indeterminate\) in order to put the authorisation on hold until further information is collected or until behavioural changes are detected.
...
Clustering of \acrshort{ucsp} and proper session management in clusters are among our architectural objectives.
We aim to analyse different clustering architectures for cloud and \gls{iot} environments and to study possible approaches of session management.
...
In addition, there are interoperability problems that must be solved in order for \gls{ssi} technologies to coexist with each other and with federated identities.
